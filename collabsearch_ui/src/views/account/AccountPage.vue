...
//   return convertBinaryToImageUrl(avatar.value)
// })
...
async function updateAvatar(files) {
  ...
  try {
    loading.value = true
    const data = await accountUpdateService(uid.value, '', '', '', content)
    updMsg.value = data.Msg
    messager.value = true
  } catch (error) {
    console.log(error)
...
<template>
  <v-card class="w-100" height="auto"
    ><v-sheet class="mx-auto w-75">
      ...
      >
        <v-avatar size="90" @click="dialog = true" class="mb-4">
          <v-img alt="User Avatar" :src="avatar" elevation="3">
