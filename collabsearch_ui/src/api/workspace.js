...
export const workspaceCreateService = (topic, description, handler, due) => {
  return instance.post(groupPath, { topic, description, handler, due })
}

export const workspaceGetJoinedService = (uid) => {
  return instance.get(groupPath + '/user' + '/' + uid)
}
...
export const workspaceGetParticipantsService = (wid) => {
  return instance.get(groupPath + '/' + wid + '/' + 'participants')
}
